...
export class Terrain {
    ...
    public audioFactor: number;

    private noiseFunction: SimplexNoise.NoiseFunction3D;
    ...
    constructor(height: number, width: number, cameraPosition: Vec3, gui: GUI) {
        this.height = height;
        this.width = width;
    ...
    private concatenateIndices(): void {
        let indices: number[] = [];
    ...
    private updateSegments(deltaTime, bassAverage, midAverage) {
...
class TerrainSegment {
    ...
    constructor(width: number, height: number, xStart: number, zStart: number, noise: SimplexNoise.NoiseFunction3D, gui: GUI) {
        this.gui = gui;
        this.vertexFactor = this.gui.vertexFactor;
    ...
    public updateVertices(deltaTime: number, bassAverage: number, midAverage: number): void {
        this.time += deltaTime;
        let index = 0;
        ...
        for (let j = 0; j <= this.width; j++) {
            ...
            const x = this.position.x + (j / this.vertexFactor);
            const z = this.position.z;
            const calculateHeight = (x: number, z: number) => {
    ...
    public updatePosition(deltaZ: number) {
    ...
    public recycleAndGenerate(newZ: number, deltaTime: number, bassAverage: number, midAverage: number) {
