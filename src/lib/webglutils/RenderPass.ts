...
export class RenderPass {
  ...
  public updateVertPosition(newData: BufferData) {
    const gl = this.ctx;
    
    // Retrieve the attribute buffer for 'vertPosition'
    const attributeBuffer = this.attributeBuffers.get('vertPosition');
    if (!attributeBuffer) {
        console.error("Vertex position buffer not found");
        return;
    }

    gl.bindBuffer(gl.ARRAY_BUFFER, attributeBuffer.bufferId);
    
    gl.bufferSubData(gl.ARRAY_BUFFER, 0, newData);
    attributeBuffer.data = newData;
  ...
  public updateNormals(newData: BufferData) {
    const gl = this.ctx;
    
    // Retrieve the attribute buffer for 'normal'
    const attributeBuffer = this.attributeBuffers.get('normal');
    if (!attributeBuffer) {
        console.error("Normal position buffer not found");
        return;
    }
